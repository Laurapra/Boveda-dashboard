...
function TarifaModal({ isOpen, onClose, userName, current, onSave }: TarifaModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Editar tarifas" subtitle={`Usuario: ${userName}`} maxWidth={420}
      footer={
        <>
          <button onClick={onClose} style={{ padding:"9px 16px", border:"1px solid var(--border)", borderRadius:"var(--radius-sm)", background:"var(--surface)", color:"var(--t1)", fontWeight:600, cursor:"pointer" }}>Cancelar</button>
  ...
  return (
    <div style={{ animation:"fadeUp .3s ease" }}>
      ...
      {tab === "onboardings" && (
        <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--radius)", overflow:"hidden", boxShadow:"var(--shadow)" }}>
          <div style={{ padding:"14px 18px", borderBottom:"1px solid var(--border)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            <button onClick={loadOnboardings} style={{ padding:"6px 12px", border:"1px solid var(--border)", borderRadius:"var(--radius-sm)", background:"var(--surface)", color:"var(--t2)", fontSize:"12px", cursor:"pointer" }}>
              <i className="ti ti-refresh" /> Actualizar
            </button>
          </div>
          {loadingOb ? (
            <div style={{ padding:"48px", textAlign:"center", color:"var(--t3)" }}>Cargando…</div>
            ...
            <div style={{ overflowX:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead>
                  <tr>
                    {["Usuario","Email","Tipo","Estado","Enviado","Acciones"].map(h => (
                      <th key={h} style={thStyle}>{h}</th>
                    ))}
                  </tr>
                </thead>
      ...
      >
        ...
        {formError && (
          <div style={{ padding:"10px 14px", background:"var(--error-dim)", border:"1px solid rgba(239,68,68,.25)", borderRadius:"var(--radius-sm)", fontSize:"13px", color:"var(--error)" }}>{formError}</div>
        )}
      </Modal>
