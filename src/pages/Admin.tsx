...
function TarifaModal({ isOpen, onClose, userName, current, onSave }: TarifaModalProps) {
  ...
  return (
    ...
    >
      ...
      {error ? <div style={{ padding: "10px 14px", background: "var(--error-dim)", borderRadius: "var(--radius-sm)", fontSize: "13px", color: "var(--error)" }}>{error}</div> : null}
    </Modal>
  );
}
...
function DocModal({ isOpen, onClose, userId, userName }: DocModalProps) {
  ...
  const loadDocs = useCallback(async () => {
    setLoading(true);
    try {
      const { data: files } = await supabase.storage.from("onboarding-docs").list(userId);
      ...
      const signed = await Promise.all(
        files.map(async (f) => {
          const { data } = await supabase.storage.from("onboarding-docs").createSignedUrl(userId + "/" + f.name, 3600);
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "32px", color: "var(--t3)" }}>
          <i className="ti ti-files" style={{ fontSize: "28px", display: "block", marginBottom: "10px", opacity: 0.3 }} />
          ...
          {docs.map((d) => (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: "13px", fontWeight: 500, color: "var(--t1)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.name.replace(/-\d+\./, ".")}</div>
                <div style={{ fontSize: "11px", color: "var(--t3)", marginTop: "2px" }}>{d.name.endsWith(".pdf") ? "PDF" : "Imagen"} · Clic para abrir</div>
              </div>
...
export const AdminView: React.FC<Props> = ({ onToast }) => {
  ...
  const loadOnboardings = useCallback(async () => {
    setLoadingOb(true);
    try {
      const { data } = await supabase.rpc("get_all_onboardings");
      setOnboardings((data ?? []) as OnboardingRow[]);
    } catch (err) {
      console.error("Onboardings catch:", getErrorMessage(err));
    } finally {
      setLoadingOb(false);
    }
  }, []);
  ...
  const loadBalances = useCallback(async () => {
    ...
    try {
      ...
      if (balanceRes && balanceRes.success && balanceRes.data) {
        setRealBalance(Number(balanceRes.data.balance) || 0);
      } else {
        setRealBalance(null);
        setBalanceError((balanceRes && (balanceRes.error || balanceRes.message)) || "No se pudo consultar el saldo real");
      }
    ...
    <div style={{ animation: "fadeUp .3s ease" }}>
      ...
      <div style={{ display: "flex", gap: "4px", borderBottom: "1px solid var(--border)", marginBottom: "18px" }}>
        {(
          [
            ["usuarios", "Usuarios"],
            ["onboardings", "Onboardings" + (pendingOb > 0 ? " (" + pendingOb + ")" : "")],
            ["saldos", "Saldos"],
          ] as [TabKey, string][]
        ).map(([key, label]) => (
          <button
        ...
        <React.Fragment>
          <div style={{ display: "flex", gap: "10px", marginBottom: "14px", alignItems: "center" }}>
            <div style={{ position: "relative", flex: 1 }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14" style={{ position: "absolute", left: "10px", top: "50%", transform: "translateY(-50%)", color: "var(--t3)" }}>
            ...
            <button onClick={() => setCreateOpen(true)} style={{ display: "inline-flex", alignItems: "center", gap: "7px", padding: "9px 16px", background: "var(--accent)", color: "#fff", border: "none", borderRadius: "var(--radius-sm)", fontWeight: 600, fontSize: "13px", cursor: "pointer" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      ...
                      filtered.map((u) => (
                        <tr key={u.id} onMouseEnter={(e) => (e.currentTarget.style.background = "var(--elevated)")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                          <td style={tdStyle}>
                            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                              <div
                                style={{
                                  ...
                                  height: "32px",
                                  borderRadius: "8px",
                                  background: u.role === "admin" ? "linear-gradient(135deg,var(--warning),#f59e0b)" : "linear-gradient(135deg,#2dd4bf,var(--accent))",
                                  display: "grid",
                                  placeItems: "center",
                                  fontWeight: 700,
                                  fontSize: "11px",
                          ...
                          <td style={{ ...tdStyle, fontSize: "12px", color: "var(--t3)" }}>{new Date(u.created_at).toLocaleDateString("es-CO", { day: "2-digit", month: "short", year: "numeric" })}</td>
                          <td style={tdStyle}>
      ...
      {tab === "onboardings" ? (
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius)", overflow: "hidden", boxShadow: "var(--shadow)" }}>
          <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <button onClick={loadOnboardings} style={{ padding: "6px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", background: "var(--surface)", color: "var(--t2)", fontSize: "12px", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: "5px" }}>
          ...
          {loadingOb ? (
            ...
            <div style={{ padding: "48px", textAlign: "center", color: "var(--t3)" }}>
              <i className="ti ti-user-check" style={{ fontSize: "28px", display: "block", marginBottom: "12px", opacity: 0.3 }} />
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {onboardings.map((o) => {
                    ...
                    return (
                      <tr key={o.id} onMouseEnter={(e) => (e.currentTarget.style.background = "var(--elevated)")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                        <td style={{ ...tdStyle, fontWeight: 500, color: "var(--t1)" }}>{o.user_name}</td>
                        <td style={{ ...tdStyle, fontSize: "12px", color: "var(--t3)" }}>{o.user_email}</td>
                        <td style={tdStyle}>
                          <span style={{ padding: "2px 8px", borderRadius: "20px", fontSize: "11px", fontWeight: 600, background: o.type === "pn" ? "var(--info-dim)" : "var(--accent-dim)", color: o.type === "pn" ? "var(--info)" : "var(--accent)" }}>{o.type === "pn" ? "Persona Natural" : "Empresa"}</span>
                        </td>
                        <td style={tdStyle}>
                          <span style={{ padding: "2px 8px", borderRadius: "20px", fontSize: "11px", fontWeight: 500, background: s.bg, color: s.color }}>{s.label}</span>
                        </td>
                        <td style={{ ...tdStyle, fontSize: "12px", color: "var(--t3)" }}>{new Date(o.submitted_at).toLocaleDateString("es-CO", { day: "2-digit", month: "short", year: "numeric" })}</td>
                        <td style={tdStyle}>
                          <button onClick={() => setDocTarget({ id: o.user_id, name: o.user_name })} style={{ display: "inline-flex", alignItems: "center", gap: "5px", padding: "5px 10px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", background: "var(--surface)", color: "var(--t2)", fontSize: "12px", fontWeight: 600, cursor: "pointer" }}>
                        ...
                        <td style={tdStyle}>
                          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                            {o.status !== "approved" ? (
                              <button onClick={() => reviewOnboarding(o.id, o.type, "approved")} style={{ padding: "4px 10px", border: "1px solid var(--success)", borderRadius: "var(--radius-sm)", background: "var(--success-dim)", color: "var(--success)", fontSize: "11px", fontWeight: 600, cursor: "pointer" }}>
                                Aprobar
                              </button>
                            ) : null}
                            {o.status !== "rejected" ? (
        ...
        <React.Fragment>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", marginBottom: "18px" }}>
            ...
            <div style={{ background: "var(--surface)", border: "1px solid var(--accent-ring)", borderRadius: "var(--radius)", padding: "20px", boxShadow: "var(--shadow)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                <i className="ti ti-building-bank" style={{ color: "var(--accent)", fontSize: "16px" }} />
                <span style={{ fontSize: "12px", fontWeight: 600, color: "var(--t3)", textTransform: "uppercase", letterSpacing: ".5px" }}>Saldo real · Cuenta Bepay</span>
              </div>
              {balanceLoading ? (
                <div style={{ fontSize: "13px", color: "var(--t3)" }}>Consultando…</div>
              ) : balanceError ? (
                <div style={{ fontSize: "12.5px", color: "var(--error)" }}>{balanceError}</div>
              ) : (
          ...
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius)", overflow: "hidden", boxShadow: "var(--shadow)" }}>
            ...
            {balanceLoading ? (
              ...
              <div style={{ padding: "48px", textAlign: "center", color: "var(--t3)" }}>
                <i className="ti ti-wallet-off" style={{ fontSize: "28px", display: "block", marginBottom: "12px", opacity: 0.3 }} />
      ...
      >
        <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--t3)", textTransform: "uppercase", letterSpacing: ".5px", paddingBottom: "8px", borderBottom: "1px solid var(--border)", marginBottom: "14px" }}>Datos de acceso</div>
        ...
        <div style={{ marginBottom: "14px" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            {[
              { v: "operator", label: "Operador", desc: "Puede operar la plataforma" },
              { v: "viewer", label: "Visor", desc: "Solo lectura" },
            ].map((opt) => (
              <button key={opt.v} onClick={() => ff("role")(opt.v)} style={{ flex: 1, padding: "10px 12px", border: "1.5px solid " + (form.role === opt.v ? "var(--accent)" : "var(--border)"), borderRadius: "var(--radius-sm)", background: form.role === opt.v ? "var(--accent-dim)" : "transparent", cursor: "pointer", textAlign: "left" }}>
        ...
        <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--t3)", textTransform: "uppercase", letterSpacing: ".5px", paddingBottom: "8px", borderBottom: "1px solid var(--border)", marginBottom: "14px", marginTop: "6px" }}>Tarifas personalizadas</div>
        ...
        {formError ? <div style={{ padding: "10px 14px", background: "var(--error-dim)", border: "1px solid rgba(239,68,68,.25)", borderRadius: "var(--radius-sm)", fontSize: "13px", color: "var(--error)" }}>{formError}</div> : null}
      </Modal>
      ...
      {/* ── Modal ver documentos ── */}
      {docTarget ? <DocModal isOpen={!!docTarget} onClose={() => setDocTarget(null)} userId={docTarget.id} userName={docTarget.name} /> : null}
    </div>
  );
};