...
export const BilleterasView: React.FC<Props> = ({ fmt, onToast }) => {
      ...
      <div style={{ animation: "fadeUp .3s ease" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "18px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                <span style={{ fontSize: "20px", fontWeight: 700, color: "var(--t1)" }}>{wallet.divisa}</span>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: "24px", fontWeight: 700, fontVariantNumeric: "tabular-nums", color: "var(--t1)" }}>{fmt(saldo)}</div>
          </div>
        </div>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius)", boxShadow: "var(--shadow)" }}>
          <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <span style={{ fontSize: "14px", fontWeight: 600, color: "var(--t1)" }}>Historial transaccional</span>
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {txns.length === 0 ? (
                  ...
                  >
                    <td style={tdStyle}>
                      <div style={{ fontFamily: "var(--mono)", fontSize: "11px", fontWeight: 500, color: "var(--t2)" }}>{t.id}</div>
                      <div style={{ fontSize: "10px", color: "var(--t3)", marginTop: "2px" }}>{fmtFechaHora(t.fecha)}</div>
                    </td>
                    <td style={{ ...tdStyle, fontSize: "12px", color: "var(--t1)" }}>{t.desc}</td>
                    <td style={tdStyle}><StatusBadge value={t.tipo} /></td>
                    <td style={{ ...tdStyle, textAlign: "right", fontWeight: 600, fontVariantNumeric: "tabular-nums", color: t.tipo === "Recibido" ? "var(--success)" : "var(--error)", opacity: t.estado !== "Completado" ? 0.4 : 1 }}>
  ...
  return (
    <div style={{ animation: "fadeUp .3s ease" }}>
      ...
      {/* Encabezado con botón Bre-B */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "18px" }}>
        <div>
          <h1 style={{ fontSize: "20px", fontWeight: 700, color: "var(--t1)", letterSpacing: "-.3px" }}>Mis billeteras</h1>
          <p style={{ fontSize: "13px", color: "var(--t3)", marginTop: "3px" }}>Gestiona tus cuentas y llaves de pago</p>
        </div>
        <button
          onClick={() => setBrebModalOpen(true)}
          style={{ display: "inline-flex", alignItems: "center", gap: "7px", padding: "9px 14px", background: "var(--accent)", color: "#fff", border: "none", borderRadius: "var(--radius-sm)", fontWeight: 600, fontSize: "13px", cursor: "pointer", boxShadow: "0 4px 12px -4px var(--accent-ring)" }}
        >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "12px", marginBottom: "20px" }}>
        {wallets.map((w, i) => (
          ...
          >
            ...
            <div style={{ fontSize: "22px", fontWeight: 700, color: "var(--t1)", marginBottom: "2px" }}>{w.divisa}</div>
            <div style={{ fontSize: "11px", color: "var(--t3)", marginBottom: "10px" }}>{w.tipo} · {w.banco}</div>
            <div style={{ fontSize: "18px", fontWeight: 600, fontVariantNumeric: "tabular-nums", color: "var(--t1)" }}>{fmt(saldo)}</div>
            <div style={{ fontSize: "10px", color: "var(--t3)", marginTop: "2px" }}>Saldo disponible</div>
          </div>
        ))}
        ...
        <button
          ...
          style={{ background: "var(--elevated)", border: "1px dashed var(--border-strong)", borderRadius: "var(--radius)", padding: "18px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "140px", transition: ".12s" }}
        ...
        >
          <i className="ti ti-plus" style={{ fontSize: "24px", color: "var(--t3)", marginBottom: "8px" }} />
          <span style={{ fontSize: "12.5px", color: "var(--t3)", fontWeight: 500 }}>Nueva billetera</span>
          <span style={{ fontSize: "11px", color: "var(--t3)", marginTop: "3px", opacity: 0.7 }}>COP, USD, EUR...</span>
        </button>
      </div>
      ...
      {wallets.length === 0 && (
        <div style={{ textAlign: "center", padding: "48px 20px", color: "var(--t3)" }}>
          <div style={{ width: "56px", height: "56px", borderRadius: "16px", background: "var(--elevated)", display: "grid", placeItems: "center", margin: "0 auto 16px" }}>
            <i className="ti ti-wallet" style={{ fontSize: "26px", opacity: .4 }} />
          </div>
          <div style={{ fontWeight: 600, fontSize: "15px", marginBottom: "6px", color: "var(--t2)" }}>Aún no tienes billeteras</div>
          <div style={{ fontSize: "13px", lineHeight: 1.6 }}>
            Crea tu primera billetera haciendo clic en<br />
            <strong style={{ color: "var(--t2)" }}>+ Nueva billetera</strong>
          </div>
        </div>
      )}
      ...
      <Modal
        ...
        subtitle="Agrega una cuenta en tu portal"
        footer={
          <>
            <button onClick={() => setModalOpen(false)} style={{ padding: "9px 16px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", background: "var(--surface)", color: "var(--t1)", fontWeight: 600, cursor: "pointer" }}>
              Cancelar
            </button>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "12.5px", color: "var(--t2)" }}>
          <span style={{ display: "inline-flex", alignItems: "center", padding: "3px 9px", borderRadius: "7px", fontSize: "11px", fontWeight: 600, color: "var(--accent)", background: "var(--accent-dim)" }}>
            Única disponible
          </span>
      ...
      <Modal
        ...
        subtitle="Tu cuenta COP · Bre-B está lista"
        footer={
          <button
            onClick={() => { setDoneOpen(false); setDetailIdx(0); }}
      ...
      >
        <div style={{ textAlign: "center", padding: "12px 0" }}>
          ...
          <div style={{ fontWeight: 600, fontSize: "15px", marginBottom: "8px", color: "var(--t1)" }}>Billetera COP activa</div>
          <div style={{ fontSize: "13px", color: "var(--t2)", marginBottom: "12px" }}>Tu llave Bre-B:</div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", fontFamily: "var(--mono)", fontSize: "14px", fontWeight: 600, color: "var(--accent)" }}>
            globalcoin@breb.co
            <button
              onClick={() => { navigator.clipboard.writeText("globalcoin@breb.co"); onToast("ok", "Copiado", "globalcoin@breb.co"); }}
      ...
      {/* Modal registrar llave Bre-B — SIEMPRE al final, dentro del return */}
